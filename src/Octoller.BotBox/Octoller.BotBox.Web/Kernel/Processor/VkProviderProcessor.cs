...
namespace Octoller.BotBox.Web.Kernel.Processor {
    ...
    public class VkProviderProcessor {
        ...
        public VkProviderProcessor(
            ...
            VkDataStore vkStore, IVkApi vkApi) {
        ...
        /// <summary>
        /// Создает запись с данными пользователя, полученными со стороны вк, без привязки к пользователю
        /// </summary>
        /// <param name="loginInfo">Данные аутентификации, полученные со стороны сервиса Vk</param>
        /// <returns>Результат выполнения операции</returns>
        public async Task<IdentityResult> CreateVkAccountAsync(string userId, ExternalLoginInfo loginInfo) {
            ...
            try {
                ...
                logger.LogError(ex, "Ошибка подключения к Api VK");
            ...
            var resultCreateAccount = await this.store.CreateAccountAsync(account);

            if (resultCreateAccount) {

                var communities = await this.vkApi.Groups.GetAsync(new GroupsGetParams {
                    UserId = vkId,
        ...
        public IEnumerable<TElement> GetCommunity<TElement>(string userId,
            System.Func<Community, TElement> cast) {
